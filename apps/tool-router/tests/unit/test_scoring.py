...
def test_score_tool_partial_matches() -> None:
    """Test partial string matching in scoring."""
    tool = {"name": "filesystem", "description": "File operations"}
    # "file" should partially match "filesystem"
    assert score_tool("file operations", "", tool) > 0
...
def test_pick_best_tools_sorting() -> None:
    ...
    # First result should have "web" and "search" in name
    assert "search" in best[0]["name"].lower()
    assert "web" in best[0]["name"].lower() or "web" in best[0]["description"].lower()
