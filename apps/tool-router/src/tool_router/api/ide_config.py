...
def generate_ide_config(
    ...
    if ide not in ("windsurf", "cursor"):
        msg = f"ide must be 'windsurf' or 'cursor', got: {ide}"
        raise ValueError(msg)
    if not server_name:
        msg = "server_name must be a non-empty string"
        raise ValueError(msg)
    if not server_uuid:
        msg = "server_uuid must be a non-empty string"
        raise ValueError(msg)
    if not gateway_url:
        msg = "gateway_url must be a non-empty string"
        raise ValueError(msg)
    if jwt_token is not None and not jwt_token:
        msg = "jwt_token must be None or a non-empty string"
        raise ValueError(msg)
