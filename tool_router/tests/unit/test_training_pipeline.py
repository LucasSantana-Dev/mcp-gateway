...
from tool_router.training.knowledge_base import KnowledgeBase, KnowledgeItem, KnowledgeStatus
...
class TestTrainingPipeline:
    ...
    def test_run_training_pipeline_extraction_error(self):
        ...
        with patch.object(self.pipeline.extractor, 'extract_from_multiple_sources', side_effect=Exception("Extraction failed")):
    ...
    def test_extract_patterns(self):
        ...
        with patch.object(self.pipeline.extractor, 'extract_from_multiple_sources', return_value=self.sample_patterns) as mock_extract:
    ...
    def test_run_continuous_learning(self):
        ...
        with patch.object(self.pipeline, '_extract_patterns', return_value=self.sample_patterns) as mock_extract:
            with patch.object(self.pipeline, '_validate_patterns', return_value=self.sample_patterns) as mock_validate:
                with patch.object(self.pipeline, '_populate_knowledge_base', return_value=["id1", "id2"]) as mock_populate:
    ...
    def test_pipeline_error_recovery(self):
        ...
        with patch.object(self.pipeline.extractor, 'extract_from_multiple_sources', side_effect=Exception("Network error")):
