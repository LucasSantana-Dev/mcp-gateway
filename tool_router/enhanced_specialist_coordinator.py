...
from .training.evaluation import SpecialistEvaluator, EvaluationMetric
...
from .ai.enhanced_selector import EnhancedAISelector, AIModel
from .ai.prompt_architect import PromptArchitect, TaskType
from .ai.ui_specialist import UISpecialist, ComponentType, UIFramework, DesignSystem, AccessibilityLevel
...
class EnhancedSpecialistCoordinator(SpecialistCoordinator):
    ...
    def _get_specialist_type_for_category(self, category: TaskCategory) -> Optional[SpecialistType]:
            ...
            if self._performance_history:
                ...
                for specialist_type in SpecialistType:
                    ...
                    if specialist_performances:
                        avg_confidence = sum(p["confidence"] for p in specialist_performances) / len(specialist_performances)
                        avg_time = sum(p["processing_time"] for p in specialist_performances) / len(specialist_performances)
    ...
    def update_specialist_with_training(self, specialist_type: SpecialistType) -> bool:
        ...
        try:
            ...
            elif specialist_type == SpecialistType.PROMPT_ARCHITECT:
                patterns = self.knowledge_base.get_patterns_by_category(
                    PatternCategory.PROMPT_ENGINEERING,
                    limit=50
                )
