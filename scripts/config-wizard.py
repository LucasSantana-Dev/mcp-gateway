...
class ConfigWizard:
        ...
        for secret, description in secrets.items():
            ...
            if secret.endswith("_SECRET"):
                if current_value and len(current_value) >= 32:
                    console.print(f"  ✅ {description}: [green]Already configured[/green]")
                else:
                    console.print(f"  🔄 {description}: [yellow]Generating new secret...[/yellow]")
                    ...
                    console.print(f"  ✅ {description}: [green]Generated[/green]")
            else:
                if current_value:
                    console.print(f"  ✅ {description}: [green]{current_value}[/green]")
                    ...
                    self.config[secret] = new_email
                    console.print(f"  ✅ {description}: [green]{new_email}[/green]")
        ...
        for ide in selected_ides:
            ...
            try:
                ...
                if result.returncode == 0:
                    console.print(f"  ✅ {ide}: [green]Configuration installed[/green]")
                else:
                    console.print(f"  ❌ {ide}: [red]Failed to install[/red]")
                    console.print(f"     Error: {result.stderr}")
            except Exception as e:
                console.print(f"  ❌ {ide}: [red]Error: {e}[/red]")
    ...
    def configure_services(self):
        ...
        if services_file.exists():
            try:
                ...
                console.print("[green]✅ Service configuration updated[/green]")
    ...
    def setup_development_environment(self):
        ...
        with Progress(
            ...
            try:
                ...
                progress.update(task, description="✅ Pre-commit hooks installed")
            except subprocess.CalledProcessError:
                progress.update(task, description="❌ Failed to install pre-commit hooks")
            ...
            if (self.repo_root / "package.json").exists():
                try:
                    ...
                    progress.update(task, description="✅ Node.js dependencies installed")
                except subprocess.CalledProcessError:
                    progress.update(task, description="❌ Failed to install Node.js dependencies")
            else:
                progress.update(task, description="⏭️ No Node.js dependencies found")
            ...
            if (self.repo_root / "requirements.txt").exists():
                try:
                    ...
                    progress.update(task, description="❌ Failed to install Python dependencies")
            else:
                progress.update(task, description="⏭️ No Python dependencies found")
    ...
    def show_summary(self):
        ...
        key_settings = {
            "JWT_SECRET_KEY": "✅ Configured" if self.config.get("JWT_SECRET_KEY") else "❌ Missing",
            "AUTH_ENCRYPTION_SECRET": "✅ Configured" if self.config.get("AUTH_ENCRYPTION_SECRET") else "❌ Missing",
            "PLATFORM_ADMIN_EMAIL": self.config.get("PLATFORM_ADMIN_EMAIL", "Not set"),
            "IDE Configurations": "✅ Setup complete" if self.detect_ides() else "⏭️ Skipped"
        }
        ...
        console.print("• [green]make help[/green] - Show contextual help with examples")
        console.print("• [green]make help-examples[/green] - Show practical examples")
        console.print("• [green]make status --detailed[/green] - Detailed system status")
        console.print("• [green]make list-servers[/green] - List virtual servers")
